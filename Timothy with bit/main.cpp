...
public:
    ...
    int* presumSize;
    bit_vector mergedBit;
    select_support_mcl<0> select_0_type;
    select_support_mcl<1> select_1_type;
    rank_support_v5<0> bit_rank_0;
    rank_support_v5<1> bit_rank_1;
...
//        cout << endl;
    ...
    toolBox.mergedBit = mergedBit;
    toolBox.select_0_type = select_0_type;
    toolBox.select_1_type = select_1_type;
    toolBox.bit_rank_0 = bit_rank_0;
    toolBox.bit_rank_1 = bit_rank_1;
...
pair<int, int> bitMethod( int start_index, int end_index, int s, int t) {
    ...
    if (bi >= bj) {
        ...
        int pos_bj = toolBox.select_1_type(bj - bi + 1 + toolBox.bit_rank_1 (toolBox.presumSize[bi])) - toolBox.presumSize[bi];
        int span_mode = 0;
        int span_freq = toolBox.bit_rank_0 (pos_bj + toolBox.presumSize[bi]) - toolBox.bit_rank_0 (toolBox.presumSize[bi]);

        int pos_last = toolBox.select_0_type(span_freq + toolBox.bit_rank_0 (toolBox.presumSize[bi])) - toolBox.presumSize[bi];
        int b_last = toolBox.bit_rank_1 (pos_last + toolBox.presumSize[bi]) - toolBox.bit_rank_1 (toolBox.presumSize[bi]) + bi;
