...
public:
    ...
    rrr_vector<> *Qk;
    rrr_vector<>::select_0_type *Qk_select_0;
    rrr_vector<>::rank_1_type   *Qk_rank_1;
    rrr_vector<> **Kr;
    rrr_vector<>::select_1_type **Kr_select_1;
...
void fouthApproximate(double epsilon, int len) {
    ...
    rrr_vector<> *Qk = new rrr_vector<>[Qk_len];
    rrr_vector<>::select_0_type *Qk_select_0 = new rrr_vector<>::select_0_type[Qk_len];
    rrr_vector<>::rank_1_type   *Qk_rank_1 = new  rrr_vector<>::rank_1_type[Qk_len];
    ...
    for (int k = 0; k < Qk_len; k++) {
        ...
        rrr_vector<> rrrb(bk);
        Qk[k] = rrrb;
        rrr_vector<>::select_0_type rrrb_select_0(&Qk[k]);
        rrr_vector<>::rank_1_type rrrb_rank_1(&Qk[k]);
        Qk_select_0[k] = rrrb_select_0;
        Qk_rank_1[k] = rrrb_rank_1;
    }

    toolBox.Qk = Qk;
    toolBox.Qk_select_0 = Qk_select_0;
    ...
    rrr_vector<> **Kr = new rrr_vector<>*[k_upper_bound];
    for (int i = 0; i < k_upper_bound; i++) {
        Kr[i] = new rrr_vector<>[2];
    }
    rrr_vector<>::select_1_type **Kr_select_1 = new rrr_vector<>::select_1_type*[k_upper_bound];
    for (int i = 0; i < k_upper_bound; i++) {
        Kr_select_1[i] = new rrr_vector<>::select_1_type[2];
    }
    rrr_vector<>::rank_0_type **Kr_rank_0 = new rrr_vector<>::rank_0_type*[k_upper_bound];
    for (int i = 0; i < k_upper_bound; i++) {
    ...
    for (int k = 0; k < k_upper_bound; k++) {
        ...
        rrr_vector<> rrrb(bit_ri);
        Kr[k][0] = rrrb;
        rrr_vector<>::select_1_type rrrb_select_1 (&Kr[k][0]);
        rrr_vector<>::rank_0_type rrrb_rank_0 (&Kr[k][0]);
        Kr_select_1[k][0] = rrrb_select_1;
        Kr_rank_0[k][0] = rrrb_rank_0;
        ...
        rrr_vector<> rrrb2(bit_rj);
        Kr[k][1] = rrrb2;

        rrr_vector<>::select_1_type rrrb2_select_1 (&Kr[k][1]);
        rrr_vector<>::rank_0_type rrrb2_rank_0 (&Kr[k][1]);
        Kr_select_1[k][1] = rrrb2_select_1;
        Kr_rank_0[k][1] = rrrb2_rank_0;
    ...
    toolBox.Kr = Kr;
    toolBox.Kr_select_1 = Kr_select_1;
...
int getRj(int curK, int j){
    int res = -1;
    //rrr_vector<> rrrb = toolBox.Kr[curK][1];
    clock_t s1, e1;
    s1 = clock();
    int pos = toolBox.Kr_select_1[curK][1](j + 1);
    e1 = clock();
    toolBox.countArray[0]++;
    toolBox.bitTime[0] += (double) (e1 - s1) / CLOCKS_PER_SEC;
...
int getQki(int k, int i) { //k: 0
    ...
    int pos_i_0 = rrrb_select_0(4); // i + 1
    int num_1 = rrrb_rank_1(pos_i_0);
    cout << "pos_i_0: " << pos_i_0 <<" num: " << num_1 << endl;*/
    // bit_vector bi = Qk[k];
    //rrr_vector<> rrrb = toolBox.Qk[k];

    //rrr_vector<>::select_0_type rrrb_select_0 (&rrrb);
    //rrr_vector<>::rank_1_type rrrb_rank_1 (&rrrb);
    //int pos_i_0 = rrrb_select_0(i + 1); // i + 1
    clock_t s1, e1;
    s1 = clock();
    int pos_i_0 = toolBox.Qk_select_0[k](i + 1);
    e1 = clock();
    toolBox.countArray[0]++;
